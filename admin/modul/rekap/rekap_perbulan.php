...
$walas = mysqli_fetch_assoc($walikelas);

if (!$walas) {
    die('Wali kelas tidak ditemukan.');
}
        ...
        <div class="col-md-12 col-xs-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    foreach ($qry as $bulanRow) {
                        $bulan = date('m', strtotime($bulanRow['tgl_absen']));
                        $tglBulan = $bulanRow['tgl_absen'];
                        $tglTerakhir = date('t', strtotime($tglBulan));
                    ?>
                    <div class="alert alert-warning alert-dismissible mt-2" role="alert">
                        <b class="text-warning" style="text-transform: uppercase;">BULAN <?= namaBulan($bulan) ?> <?= date('Y', strtotime($tglBulan)) ?></b>
                        <hr>
                        <p>
                            ...
                            <a target="_blank" href="../guru/modul/rekap/rekap_bulan.php?pelajaran=<?= $_GET['pelajaran'] ?>&bulan=<?= $bulan ?>&kelas=<?= $_GET['kelas'] ?>" class="btn btn-default">
                                ...
                                CETAK BULAN (<?= strtoupper(namaBulan($bulan)) ?>)
                            </a>
                        </p>
                        <table width="100%" border="1" cellpadding="2" style="border-collapse: collapse;">
                            ...
                            foreach ($qryAbsen as $absenRow) {
                                $warna = ($no % 2 == 1) ? "#ffffff" : "#f0f0f0";
                            ?>
                            <tr bgcolor="<?= $warna ?>">
                                <td align="center"><?= $no++ ?></td>
                                <td><?= $absenRow['nama_siswa'] ?></td>
                                <td align="center"><?= $absenRow['jk'] ?></td>
                                <?php 
                                for ($i = 1; $i <= $tglTerakhir; $i++) {
                                ?>
                                <td align="center" bgcolor="white">
                                    <?php 
                                    $ket = mysqli_query($con, "SELECT * FROM _logabsensi WHERE DAY(tgl_absen) = '$i' AND id_siswa = '{$absenRow['id_siswa']}' AND id_mengajar = '$_GET[pelajaran]' AND MONTH(tgl_absen) = '$bulan' GROUP BY DAY(tgl_absen)");
                                    foreach ($ket as $h) {
                                        if ($h['ket'] == 'H') {
                                ...
                                <td align="center" style="font-weight: bold;">
                                    <?php 
                                    $sakit = mysqli_fetch_array(mysqli_query($con, "SELECT COUNT(ket) AS sakit FROM _logabsensi WHERE id_siswa = '{$absenRow['id_siswa']}' AND ket = 'S' AND MONTH(tgl_absen) = '$bulan' AND id_mengajar = '$_GET[pelajaran]'"));
                                    echo $sakit['sakit'];
                                    ?>
                                </td>
                                <td align="center" style="font-weight: bold;">
                                    <?php 
                                    $izin = mysqli_fetch_array(mysqli_query($con, "SELECT COUNT(ket) AS izin FROM _logabsensi WHERE id_siswa = '{$absenRow['id_siswa']}' AND ket = 'I' AND MONTH(tgl_absen) = '$bulan' AND id_mengajar = '$_GET[pelajaran]'"));
                                    echo $izin['izin'];
                                    ?>
                                </td>
                                <td align="center" style="font-weight: bold;">
                                    <?php 
                                    $alfa = mysqli_fetch_array(mysqli_query($con, "SELECT COUNT(ket) AS alfa FROM _logabsensi WHERE id_siswa = '{$absenRow['id_siswa']}' AND ket = 'A' AND MONTH(tgl_absen) = '$bulan' AND id_mengajar = '$_GET[pelajaran]'"));
                                    echo $alfa['alfa'];
                                    ?>
                                </td>
                            </tr>
