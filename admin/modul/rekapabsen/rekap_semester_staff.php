...
// Tentukan bulan awal dan akhir untuk masing-masing semester berdasarkan pilihan pengguna
if ($semester == 'ganjil') {
    $bulan_awal = 7;
    $bulan_akhir = 12;
    // Untuk semester ganjil, tahun ajaran tetap sesuai dengan tahun yang dipilih
    $tahun_ajaran = $_POST['tahun_ajaran'];
} elseif ($semester == 'genap') {
    $bulan_awal = 1;
    $bulan_akhir = 6;
    // Untuk semester genap, tahun ajaran adalah tahun sebelumnya
    $tahun_ajaran = $_POST['tahun_ajaran'];
} else {
    // Handle jika pilihan semester tidak valid
    echo "Pilihan semester tidak valid!";
    exit;
}


// Ambil nilai tahun ajaran yang dipilih oleh pengguna dari formulir
$tahun_ajaran = $_POST['tahun_ajaran'];
...
$info_thajaran = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM tb_thajaran WHERE tahun_ajaran = '$tahun_ajaran' AND status = 1"));
...
<head>
    ...
    <title>Rekap Absensi Staff - Semester <?= $semester ?> Tahun Ajaran <?= $tahun_ajaran ?></title>
...
<body>
        ...
        <tr>
            ...
            <td>
                <center>
                    <h1>
                        <small>
                            <b>REKAP ABSENSI STAFF <br> PER SEMESTER <br>
                            SMK GALUNGGUNG</b>
                        </small>
                    </h1>
            ...
            <td>
                <table width="100%" class="info-table" style="border-collapse: collapse; margin-bottom:100px; margin-left: auto; margin-right:20px; margin-top:15px;">
                    ...
                    <tr>
                        ...
                        <td style="border: 0.5px solid; padding: 8px;"><?= $tahun_ajaran ?></td>
                    </tr>
                </table>
            </td>
        </tr>
    ...
    <?php
// Loop untuk setiap bulan dalam satu semester
for ($bulan = $bulan_awal; $bulan <= $bulan_akhir; $bulan++) {
    // Hitung jumlah hari dalam bulan yang dipilih
    $tglTerakhir = date('t', strtotime($tahun_ajaran . '-' . $bulan . '-01'));
    // Tambahkan kode rekap absensi guru di sini

        $tglTerakhir = date('t', strtotime($tahun_ajaran . '-' . $bulan . '-01'));
        ?>

    <table width="100%" border="1" cellpadding="2" style="border-collapse: collapse; margin-bottom: 50px;">
        <!-- Isi tabel rekap absensi guru -->
        <tr>
            <td rowspan="2" bgcolor="#EFEBE9" align="center">NO</td>
            <td rowspan="2" bgcolor="#EFEBE9">NAMA STAFF</td>
            <td colspan="<?= $tglTerakhir; ?>" style="padding: 8px;">BULAN : <b style="text-transform: uppercase;"><?php echo namaBulan($bulan); ?> <?= $tahun_ajaran ?></b></td>
        ...
        foreach ($staff as $s) {
            ...
            for ($i = 1; $i <= $tglTerakhir; $i++) {
                $ket = mysqli_query($con, "SELECT * FROM tb_absensi_staff
                WHERE DAY(tanggal)='$i' AND id_staff='$ds[id_staff]' AND MONTH(tanggal)='$bulan' AND id_semester = $id_semester AND id_thajaran IN (SELECT id_thajaran FROM tb_thajaran WHERE status=1)");
                ...
                // Tampilkan baris tabel untuk setiap staff
                ?>
                <tr bgcolor="<?= $warna; ?>">
