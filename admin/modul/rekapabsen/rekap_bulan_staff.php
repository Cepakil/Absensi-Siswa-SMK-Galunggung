...
<head>
    ...
    <title>Rekap Absensi Staff - <?= namaBulan(date('m', strtotime($bulan))) ?> <?= date('Y', strtotime($bulan)) ?></title>
    ...
    <table width="100%" border="1" cellpadding="2" style="border-collapse: collapse;">
        ...
        foreach ($staff as $s) {
            ...
            for ($i = 1; $i <= $tglTerakhir; $i++) {
                ...
                WHERE DAY(tanggal)='$i' AND id_staff='$ds[id_staff]' AND MONTH(tanggal)='".date('m', strtotime($bulan))."' AND id_semester = $id_semester AND id_thajaran IN (SELECT id_thajaran FROM tb_thajaran WHERE status=1)");
