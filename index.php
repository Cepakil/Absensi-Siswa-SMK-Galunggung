...
<head>
    ...
    <style>
        .login100-form-btn {
            background-color: #539A55; /* Warna hijau */
            color: white;
        }

        .login100-form-btn:hover {
            background-color: #3F8E43; /* Warna hijau lebih gelap saat hover */
        }
    </style>
</head>
<body>
            ...
            <div class="wrap-login100">
                <form method="post" action="" class="login100-form validate-form">
                    <span class="login100-form-title p-b-48">
                        <img src="./assets/img/logo1.png" width="110">
                    </span>
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    if ($level == 1) {
                        ...
                        // Wali Kelas
                        $sqlCek = mysqli_query($con, "SELECT * FROM tb_walikelas WHERE nip='$_POST[username]' AND password='$_POST[password]'");
                        $jml = mysqli_num_rows($sqlCek);
                        $d = mysqli_fetch_array($sqlCek);

                        if ($jml > 0) {
                            $_SESSION['wakel'] = $d['id_walikelas'];
                            echo "
                            <script type='text/javascript'>
                            setTimeout(function () {
                            swal('($d[nama_walikelas]) ', 'Login berhasil', {
                            icon : 'success',
                            buttons: {
                            confirm: {
                            className : 'btn btn-success'
                        ...
                        // Orang Tua
                        $sqlCek = mysqli_query($con, "SELECT * FROM tb_orangtua WHERE username='$_POST[username]' AND password='$pass' AND status='1'");
                        $jml = mysqli_num_rows($sqlCek);
                        $d = mysqli_fetch_array($sqlCek);

                        if ($jml > 0) {
                            $_SESSION['orangtua'] = $d['id_orangtua'];
                            echo "
                            <script type='text/javascript'>
                            setTimeout(function () {
                            swal('($d[nama_orangtua]) ', 'Login berhasil', {
                            icon : 'success',
                            buttons: {
                            confirm: {
                            className : 'btn btn-success'
                            ...
                            window.setTimeout(function(){
                            window.location.replace('./orangtua/');
                            }, 3000);
                            </script>";
                        } else {
                            echo "
